...
def create_seams(_map, side_length):
    ...
    for (face, side), edge in edges.items():
        ...
        seam_facing1 = side_facing(side)
        pos1 = start_edge_coordinates(face, side, side_length, seam_facing1)
        dir1 = facing_direction(seam_facing1)
        seam_facing2 = side_facing(pair_side)
        if edge != pair_edge:
            seam_facing2 = opposite_facing(seam_facing2)
        pos2 = start_edge_coordinates(pair_face, pair_side, side_length, seam_facing2)
        dir2 = facing_direction(seam_facing2)
        print(NAME[side], face, NAME[seam_facing1])
        print(NAME[pair_side], pair_face, NAME[seam_facing2])
        print()
